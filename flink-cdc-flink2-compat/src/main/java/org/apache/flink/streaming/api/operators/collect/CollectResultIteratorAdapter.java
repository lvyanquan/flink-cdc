 ...
 *
 * <p>In Flink 2.x, the operatorUid should be obtained from {@code
 * sink.getTransformation().getId()}, not from {@code operator.getOperatorID()} which returns null
...
public class CollectResultIteratorAdapter<T> extends CollectResultIterator<T> {
     ...
     *
     * @param operatorUid the operator ID string, typically from {@code
    ...
    public CollectResultIteratorAdapter(
        ...
        super(operatorUid, serializer, accumulatorName, checkpointConfig, resultFetchTimeout);
    }
    ...
    public CollectResultIteratorAdapter(
            ...
            String accumulatorName,
            int retryMillis) {
        super(buffer, operatorUid, accumulatorName, retryMillis);
    }
}
