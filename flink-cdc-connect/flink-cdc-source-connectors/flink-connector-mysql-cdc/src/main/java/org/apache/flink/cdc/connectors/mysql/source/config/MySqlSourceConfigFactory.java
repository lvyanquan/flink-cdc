...
public class MySqlSourceConfigFactory implements Serializable {
     ...
     * required. Every ID must be unique across all currently-running database processes in the
     * MySQL cluster. This connector joins the MySQL cluster as another server (with this unique ID)
     * so it can read the binlog. By default, a random number is generated between 5400 and 6400,
