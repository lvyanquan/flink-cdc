 ...
 * Event}.
 *
 * <p>Line 196: add {@link #processInParallel} to indicate that events are converted in parallel.
 *
...
public class BinaryLogClient implements BinaryLogClientMXBean {
    ...
    private static final SSLSocketFactory DEFAULT_REQUIRED_SSL_MODE_SOCKET_FACTORY =
    ...
    private static final SSLSocketFactory DEFAULT_VERIFY_CA_SSL_MODE_SOCKET_FACTORY =
            new DefaultSSLSocketFactory();
    ...
    public volatile long eventLastSeen;
     ...
     * @param hostname mysql server hostname
     * @param port mysql server port
     * @param schema database name, nullable. Note that this parameter has nothing to do with event
     *     filtering. It's used only during the authentication.
     * @param username login name
     * @param password password
     */
    public BinaryLogClient(
            String hostname, int port, String schema, String username, String password) {
    ...
    /**
     * @param blocking blocking mode. If set to false - BinaryLogClient will disconnect after the
     *     last event.
     */
    ...
    /**
     * @param serverId server id (in the range from 1 to 2^32 - 1). This value MUST be unique across
     *     whole replication group (that is, different from any other server id being used by any
     *     master or slave). Keep in mind that each binary log client
     *     (mysql-binlog-connector-java/BinaryLogClient, mysqlbinlog, etc) should be treated as a
    ...
    /**
     * @return binary log filename, nullable (and null be default). Note that this value is
     *     automatically tracked by the client and thus is subject to change (in response to {@link
     ...
     *     <ul>
     *       <li>null, which turns on automatic resolution (resulting in the last known binlog and
     *           position). This is what happens by default when you don't specify binary log
     *           filename explicitly.
     *       <li>"" (empty string), which instructs server to stream events starting from the oldest
     *           known binlog.
     *     </ul>
     *
    ...
    /**
     * @return binary log position of the next event, 4 by default (which is a position of first
     *     event). Note that this value changes with each incoming event.
    ...
    /**
     * @param binlogPosition binary log position. Any value less than 4 gets automatically adjusted
     *     to 4 on connect.
    ...
    /**
     * @return GTID set. Note that this value changes with each received GTID event (provided client
     *     is in GTID mode).
    ...
    /**
     * @param gtidStr GTID set string (can be an empty string). NOTE #1: Any value but null will
     *     switch BinaryLogClient into a GTID mode (this will also set binlogFilename to ""
     *     (provided it's null) forcing MySQL to send events starting from the oldest known binlog
     *     (keep in mind that connection will fail if gtid_purged is anything but empty (unless
     *     {@link #setGtidSetFallbackToPurged(boolean)} is set to true))). NOTE #2: GTID set is
     *     automatically updated with each incoming GTID event (provided GTID mode is on).
     * @see #getGtidSet()
    ...
    /**
     * @param gtidSetFallbackToPurged true if gtid_purged should be used as a fallback when gtidSet
     *     is set to "" and MySQL server has purged some of the binary logs, false otherwise
     *     (default).
     */
    ...
    /**
     * @param useBinlogFilenamePositionInGtidMode true if MySQL server should start streaming events
     *     from a given {@link #getBinlogFilename()} and {@link #getBinlogPosition()} instead of
     *     "the oldest known binlog" when {@link #getGtidSet()} is set, false otherwise (default).
     */
    public void setUseBinlogFilenamePositionInGtidMode(
            boolean useBinlogFilenamePositionInGtidMode) {
        this.useBinlogFilenamePositionInGtidMode = useBinlogFilenamePositionInGtidMode;
    }

    /**
     * @return true if "keep alive" thread should be automatically started (default), false
     *     otherwise.
    ...
    /**
     * @param keepAlive true if "keep alive" thread should be automatically started (recommended and
     *     true by default), false otherwise.
     * @see #isKeepAlive()
    ...
    /**
     * @param heartbeatInterval heartbeat period in milliseconds. If set (recommended)
     *     <ul>
     *       <li>HEARTBEAT event will be emitted every "heartbeatInterval".
     *       <li>if {@link #setKeepAlive(boolean)} is on then keepAlive thread will attempt to
     *           reconnect if no HEARTBEAT events were received within {@link
     *           #setKeepAliveInterval(long)} (instead of trying to send PING every {@link
     *           #setKeepAliveInterval(long)}, which is fundamentally flawed -
     *           https://github.com/shyiko/mysql-binlog-connector-java/issues/118).
     *     </ul>
     *     Note that when used together with keepAlive heartbeatInterval MUST be set less than
     *     keepAliveInterval.
    ...
    /**
     * @param socketFactory custom socket factory. If not provided, socket will be created with "new
     *     Socket()".
     */
    ...
    /**
     * @param threadFactory custom thread factory. If not provided, threads will be created using
     *     simple "new Thread()".
     */
    ...
    public void connect() throws IOException, IllegalStateException {
        ...
        try {
            ...
            try {
                    ...
                    if (connectTimeout > 0 && !isKeepAliveThreadRunning()) {
                        cancelDisconnect =
                                scheduleDisconnectIn(
                                        connectTimeout - (System.currentTimeMillis() - start));
                    }
                ...
                new Authenticator(greetingPacket, channel, schema, username, password)
                        .authenticate();
    ...
    private void enableHeartbeat() throws IOException {
        channel.write(
                new QueryCommand("set @master_heartbeat_period=" + heartbeatInterval * 1000000));
    ...
    private void requestBinaryLogStreamMysql(long serverId) throws IOException {
        ...
        synchronized (gtidSetAccessLock) {
            if (this.gtidEnabled) {
                ...
                dumpBinaryLogCommand =
                        new DumpBinaryLogCommand(serverId, binlogFilename, binlogPosition);
            }
        }
    ...
    protected void ensureEventDataDeserializer(
            EventType eventType,
        ...
        if (eventDataDeserializer.getClass() != eventDataDeserializerClass
            ...
            eventDeserializer.setEventDataDeserializer(
                    eventType,
                    new EventDataWrapper.Deserializer(
    ...
    protected void ensureGtidEventDataDeserializer() {
        ...
        ensureEventDataDeserializer(
                EventType.ANNOTATE_ROWS, AnnotateRowsEventDataDeserializer.class);
        ensureEventDataDeserializer(EventType.MARIADB_GTID, MariadbGtidEventDataDeserializer.class);
        ensureEventDataDeserializer(
                EventType.MARIADB_GTID_LIST, MariadbGtidListEventDataDeserializer.class);
    }
                        ...
                        public void run() {
                            while (!threadExecutor.isShutdown()) {
                                ...
                                if (threadExecutor.isShutdown()) {
                                    logger.info(
                                            "threadExecutor is shut down, terminating keepalive thread");
                                    return;
                                }
                                boolean connectionLost = false;
                                if (heartbeatInterval > 0) {
                                    connectionLost =
                                            System.currentTimeMillis() - eventLastSeen
                                                    > keepAliveInterval;
                                } else {
                                    try {
                                ...
                                if (connectionLost) {
                                    ...
                                    try {
                                        ...
                                        logger.warning(
                                                "keepalive: Failed to restore connection to "
                                                        + hostname
                                                        + ":"
                                                        + port
                                                        + ". Next attempt in "
    ...
    private Thread newNamedThread(Runnable runnable, String threadName) {
        Thread thread =
                threadFactory == null ? new Thread(runnable) : threadFactory.newThread(runnable);
        ...
        if (!started) {
            try {
                ...
                throw new TimeoutException(
                        "BinaryLogClient was unable to connect in " + timeout + "ms");
            }
        }
    }
        ...
        try {
            while (inputStream.peek() != -1) {
                ...
                try {
                    ...
                    Throwable cause =
                            e instanceof EventDataDeserializationException ? e.getCause() : e;
                ...
                // add processInParallel check because Event may be process in
                // BinlogEventConsumer.
                if (isConnected() && !processInParallel) {
            ...
            if (isConnected()) {
                if (completeShutdown) {
                    disconnect(); // initiate complete shutdown sequence (which includes keep alive
                    // thread)
                } else {
                    disconnectChannel();
                }
    ...
    private byte[] readPacketSplitInChunks(ByteArrayInputStream inputStream, int packetLength)
            throws IOException {
    ...
    public void updateClientBinlogFilenameAndPosition(Event event) {
        EventHeader eventHeader = event.getHeader();
        EventType eventType = eventHeader.getEventType();
        if (eventType == EventType.ROTATE) {
            RotateEventData rotateEventData =
                    (RotateEventData) EventDataWrapper.internal(event.getData());
            binlogFilename = rotateEventData.getBinlogFilename();
            binlogPosition = rotateEventData.getBinlogPosition();
        } else
        // do not update binlogPosition on TABLE_MAP so that in case of reconnect (using a different
        // instance of
        ...
        switch (eventHeader.getEventType()) {
            case GTID:
                GtidEventData gtidEventData =
                        (GtidEventData) EventDataWrapper.internal(event.getData());
            ...
            case QUERY:
                QueryEventData queryEventData =
                        (QueryEventData) EventDataWrapper.internal(event.getData());
            ...
            case ANNOTATE_ROWS:
                AnnotateRowsEventData annotateRowsEventData =
                        (AnnotateRowsEventData) EventDataWrapper.internal(event.getData());
            ...
            case MARIADB_GTID:
                MariadbGtidEventData mariadbGtidEventData =
                        (MariadbGtidEventData) EventDataWrapper.internal(event.getData());
            ...
            case MARIADB_GTID_LIST:
                MariadbGtidListEventData mariadbGtidListEventData =
                        (MariadbGtidListEventData) EventDataWrapper.internal(event.getData());
    ...
    public void notifyEventListeners(Event event) {
        if (event.getData() instanceof EventDataWrapper) {
            event =
                    new Event(
                            event.getHeader(), ((EventDataWrapper) event.getData()).getExternal());
        }
        for (EventListener eventListener : eventListeners) {
            try {
    ...
    /**
     * Register lifecycle listener. Note that multiple lifecycle listeners will be called in order
     * they where registered.
     *
    ...
    /**
     * Disconnect from the replication stream. Note that this does not cause
     * binlogFilename/binlogPosition to be cleared out. As the result following {@link #connect()}
    ...
    private static boolean awaitTerminationInterruptibly(
    ...
    public interface LifecycleListener {
        ...
        /**
         * Called once client has successfully logged in but before started to receive binlog
         * events.
         *
        ...
        /**
         * Called in case of failed event deserialization. Note this type of error does NOT cause
         * client to disconnect. If you wish to stop receiving events you'll need to fire
         * client.disconnect() manually.
         *
