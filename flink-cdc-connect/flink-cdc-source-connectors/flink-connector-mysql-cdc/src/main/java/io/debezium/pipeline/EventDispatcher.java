 ...
 *
 * <p>Line 83: Change visibility of {@link #schema} to public.
 *
 * <p>Line 88: Change visibility of {@link #heartbeat} to public.
 *
 * <p>Line 89: Change visibility of {@link #eventListener} to public.
 *
 * <p>Line 92: Change visibility of {@link #transactionMonitor} to public.
 *
 * <p>Line 94: Change visibility of {@link #skippedOperations} to public.
 *
 * <p>Line 95: Change visibility of {@link #neverSkip} to public.
 *
 * <p>Line 100: Change visibility of {@link #signal} to public.
 *
 * <p>Line 101: Change visibility of {@link #incrementalSnapshotChangeEventSource} to public.
 *
 * <p>Line 106: Change visibility of {@link #EventDispatcher} to public.
 *
...
public class EventDispatcher<P extends Partition, T extends DataCollectionId>
    ...
    public final DatabaseSchema<T> schema;
    ...
    public DataChangeEventListener<P> eventListener = DataChangeEventListener.NO_OP();
    private final boolean emitTombstonesOnDelete;
    private final InconsistentSchemaHandler<P, T> inconsistentSchemaHandler;
    public final TransactionMonitor transactionMonitor;
    private final CommonConnectorConfig connectorConfig;
    public final EnumSet<Operation> skippedOperations;
    public final boolean neverSkip;

    private final Schema schemaChangeKeySchema;
    private final Schema schemaChangeValueSchema;
    private final ConnectTableChangeSerializer tableChangesSerializer;
    public final Signal<P> signal;
    public IncrementalSnapshotChangeEventSource<P, T> incrementalSnapshotChangeEventSource;

    /** Change event receiver for events dispatched from a streaming change event source. */
    public final StreamingChangeRecordReceiver streamingReceiver;
    ...
    public EventDispatcher(
        ...
        this.inconsistentSchemaHandler =
                inconsistentSchemaHandler != null
                        ? inconsistentSchemaHandler
                        : this::errorOnMissingSchema;
        this.skippedOperations = connectorConfig.getSkippedOperations();
        this.neverSkip =
                connectorConfig.supportsOperationFiltering() || this.skippedOperations.isEmpty();
        ...
        schemaChangeValueSchema =
                SchemaBuilder.struct()
                        ...
                        .field(
                                Fields.TABLE_CHANGES,
                                SchemaBuilder.array(tableChangesSerializer.getChangeSchema())
                                        .build())
                        .build();
    }

    public void dispatchSnapshotEvent(
            P partition,
            T dataCollectionId,
            ChangeRecordEmitter<P> changeRecordEmitter,
    ...
    public SnapshotReceiver<P> getIncrementalSnapshotChangeEventReceiver(
            DataChangeEventListener<P> dataListener) {
        return new IncrementalSnapshotChangeRecordReceiver(dataListener);
    }

    /**
     * Dispatches one or more {@link DataChangeEvent}s. If the given data collection is included in
     * the currently captured set of collections, the given emitter will be invoked, so it can emit
     * one or more events (in the common case, one event will be emitted, but e.g. in case of PK
     * updates, it may be a deletion and a creation event). The receiving coordinator creates {@link
     * SourceRecord}s for all emitted events and passes them to this dispatcher's {@link
     * ChangeEventCreator} for converting them into data change events.
     *
     * @return {@code true} if an event was dispatched (i.e. sent to the message broker), {@code
     *     false} otherwise.
     */
    public boolean dispatchDataChangeEvent(
            P partition, T dataCollectionId, ChangeRecordEmitter<P> changeRecordEmitter)
            throws InterruptedException {
        try {
            ...
            if (!filter.isIncluded(dataCollectionId)) {
                ...
                eventListener.onFilteredEvent(
                        ...
                        changeRecordEmitter.getOperation());
                ...
                // TODO handle as per inconsistent schema info option
                if (dataCollectionSchema == null) {
                    final Optional<DataCollectionSchema> replacementSchema =
                            inconsistentSchemaHandler.handle(
                                    partition, dataCollectionId, changeRecordEmitter);
                        ...
                        new Receiver<P>() {
                            ...
                            public void changeRecord(
                                ...
                                if (operation == Operation.CREATE
                                        && signal.isSignal(dataCollectionId)) {
                                ...
                                if (neverSkip || !skippedOperations.contains(operation)) {
                                    ...
                                    streamingReceiver.changeRecord(
                                            partition, schema, operation, key, value, offset,
                                            headers);
                                }
                            }
                        });
    ...
    public void dispatchFilteredEvent(P partition, OffsetContext offset)
            throws InterruptedException {
        if (incrementalSnapshotChangeEventSource != null) {
    ...
    public void dispatchTransactionCommittedEvent(P partition, OffsetContext offset)
            throws InterruptedException {
        transactionMonitor.transactionComittedEvent(partition, offset);
        if (incrementalSnapshotChangeEventSource != null) {
            incrementalSnapshotChangeEventSource.processTransactionCommittedEvent(
                    partition, offset);
        }
    }

    public void dispatchTransactionStartedEvent(
            P partition, String transactionId, OffsetContext offset) throws InterruptedException {
    ...
    public Optional<DataCollectionSchema> errorOnMissingSchema(
            P partition, T dataCollectionId, ChangeRecordEmitter<P> changeRecordEmitter) {
        eventListener.onErroneousEvent(
                partition, "source = " + dataCollectionId, changeRecordEmitter.getOperation());
        throw new IllegalArgumentException(
                "No metadata registered for captured table " + dataCollectionId);
    }

    public Optional<DataCollectionSchema> ignoreMissingSchema(
            T dataCollectionId, ChangeRecordEmitter<P> changeRecordEmitter) {
        return Optional.empty();
    }

    public void dispatchSchemaChangeEvent(
            P partition, T dataCollectionId, SchemaChangeEventEmitter schemaChangeEventEmitter)
            throws InterruptedException {
    ...
    public void alwaysDispatchHeartbeatEvent(P partition, OffsetContext offset)
            throws InterruptedException {
        heartbeat.forcedBeat(
                partition.getSourcePartition(), offset.getOffset(), this::enqueueHeartbeat);
    }

    public void dispatchHeartbeatEvent(P partition, OffsetContext offset)
            throws InterruptedException {
        heartbeat.heartbeat(
                partition.getSourcePartition(), offset.getOffset(), this::enqueueHeartbeat);
    }
    ...
    public void enqueueHeartbeat(SourceRecord record) throws InterruptedException {
    ...
    public void dispatchServerHeartbeatEvent(P partition, OffsetContext offset)
            throws InterruptedException {
        if (incrementalSnapshotChangeEventSource != null) {
    ...
    /**
     * Change record receiver used during snapshotting. Allows for a deferred submission of records,
     * which is needed in order to set the "snapshot completed" offset field, which we can't send to
     * Kafka Connect without sending an actual record
     */
    public interface SnapshotReceiver<P extends Partition> extends Receiver<P> {
        void completeSnapshot() throws InterruptedException;
    }

    /** Put {@link SourceRecord} to {@link ChangeEventQueue}. */
    public final class StreamingChangeRecordReceiver implements Receiver<P> {
    ...
    private final class BufferingSnapshotChangeRecordReceiver implements SnapshotReceiver<P> {
        ...
        public void changeRecord(
            ...
            // the record is produced lazily, so to have the correct offset as per the pre/post
    ...
    public void setIncrementalSnapshotChangeEventSource(
            Optional<IncrementalSnapshotChangeEventSource<P, ? extends DataCollectionId>>
                    incrementalSnapshotChangeEventSource) {
        this.incrementalSnapshotChangeEventSource =
                (IncrementalSnapshotChangeEventSource<P, T>)
                        incrementalSnapshotChangeEventSource.orElse(null);
    }
    ...
    public interface InconsistentSchemaHandler<P extends Partition, T extends DataCollectionId> {
        ...
        /**
         * @return collection schema if the schema was updated and event can be processed, {@code
         *     empty} to skip the processing
         */
        Optional<DataCollectionSchema> handle(
