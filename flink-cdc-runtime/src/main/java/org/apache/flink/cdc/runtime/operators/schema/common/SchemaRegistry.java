...
import org.apache.flink.cdc.runtime.operators.schema.common.event.GetTokenRequest;
import org.apache.flink.cdc.runtime.operators.schema.common.event.GetTokenResponse;
...
public abstract class SchemaRegistry implements OperatorCoordinator, CoordinationRequestHandler {
    ...
    protected void handleGetTokenRequest(
            GetTokenRequest request, CompletableFuture<CoordinationResponse> responseFuture) {
        LOG.info("Handling token request: {}", request);
        TableId tableId = request.getTableId();
        responseFuture.complete(wrap(new GetTokenResponse(metadataApplier.getToken(tableId))));
    }
        ...
        runInEventLoop(
                () -> {
                    if (request instanceof GetEvolvedSchemaRequest) {
                        ...
                        handleGetTokenRequest((GetTokenRequest) request, future);
                    } else {
